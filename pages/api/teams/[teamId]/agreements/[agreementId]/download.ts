...
export default async function handle(
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      // Check if the content is a Datavault URL
      const isDatavaultUrl =
        agreement.content.includes("datavault.com/view/") ||
        agreement.content.includes("www.datavault.com/view/");
      ...
      if (isDatavaultUrl) {
        // Extract linkId from Datavault URL
        const urlParts = agreement.content.split("/view/");
        if (urlParts.length < 2) {
          return res.status(400).json("Invalid Datavault URL format");
        }
        ...
        if (!link || !link.document) {
          return res
            .status(404)
            .json("Document not found for the provided Datavault URL");
        }
