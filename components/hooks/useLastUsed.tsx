...
export function useLastUsed() {
  ...
  useEffect(() => {
    const storedValue = localStorage.getItem("last_datavault_login");
    if (storedValue) {
  ...
  useEffect(() => {
    if (lastUsed) {
      localStorage.setItem("last_datavault_login", lastUsed);
    } else {
      localStorage.removeItem("last_datavault_login");
    }
  }, [lastUsed]);
